...
_LOCATION_PREP = re.compile(
    rf"\b(?:в|во|из|из\s+города|в\s+городе|в\s+селе|в\s+ауле|"
...
_RU_RELATION = re.compile(
    ...
    rf"(?:был(?:а)?|являл(?:ся|ась)|приходил(?:ся|ась))\s+"
    rf"(?P<relation>братом|сестрой|отцом|матерью|сыном|дочерью|мужем|женой|тётей|тетей|дядей)\s+"
    rf"(?P<object>[A-ZА-ЯЁӘҒҚҢӨҰҮҺІ][{_CYR}'’-]{{2,}})",
    re.IGNORECASE,
)
_RU_PRONOUN_RELATION = re.compile(
    rf"\b(?:его|её|ее|ему|ей)\s+"
    rf"(?P<relation>брат|сестра|отец|мать|сын|дочь|тётя|тетя|дядя)\s+"
    rf"(?P<person>[A-ZА-ЯЁӘҒҚҢӨҰҮҺІ][{_CYR}'’-]{{2,}})",
    re.IGNORECASE,
)
_RU_REVERSE_PRONOUN = re.compile(
    rf"\b(?P<person>[A-ZА-ЯЁӘҒҚҢӨҰҮҺІ][{_CYR}'’-]{{2,}})\s+"
    rf"приходил(?:ся|ась)\s+(?:ему|ей)\s+"
    rf"(?P<relation>братом|сестрой|отцом|матерью|сыном|дочерью|тётей|тетей|дядей)",
    re.IGNORECASE,
)
_KZ_RELATION = re.compile(
    rf"\b(?P<object>[A-ZА-ЯЁӘҒҚҢӨҰҮҺІ][{_CYR}'’-]{{2,}}?)(?:ның|нің|дың|дің|тың|тің)\s+"
    rf"(?P<relation>ағасы|інісі|әпкесі|сіңлісі|қарындасы|әкесі|анасы|шешесі|ұлы|қызы)\s+"
