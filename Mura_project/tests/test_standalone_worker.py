...
def test_build_worker_supervisor_can_isolate_book_queue() -> None:
    supervisor = build_worker_supervisor(_settings(WORKER_QUEUES="book"))

    assert supervisor.recording_worker is None
    assert isinstance(supervisor.book_worker, BookJobWorker)
    assert supervisor.cleanup_worker is None


def test_build_worker_supervisor_can_isolate_cleanup_queue() -> None:
    supervisor = build_worker_supervisor(_settings(WORKER_QUEUES="cleanup"))
